  ...
  -- Description: A Neovim plugin for creating a customizable dashboard.
  {
    ...
    opts = {
      ...
      dashboard = {
        sections = {
          { section = "header" },
          { icon = " ", title = "Atajos", section = "keys", indent = 2, padding = 1 },
...
▝▚▄▟▌█ ▝▚▄▟▌█   █  █   █ ▀▄▄▞▘▄ █   █ ▗▄▟▙▄▖▝▚▄▟▌█   █ ▝▚▄▟▌
     ...
     █                 █      █         ▐▌                  
          ...
          keys = {
            ...
            { icon = " ", key = "g", desc = "Encontrar Texto", action = ":lua Snacks.dashboard.pick('live_grep')" },
            { icon = " ", key = "r", desc = "Arcivos Recientes", action = ":lua Snacks.dashboard.pick('oldfiles')" },
            { icon = " ", key = "c", desc = "Configuracion", action = ":lua Snacks.dashboard.pick('files', {cwd = vim.fn.stdpath('config')})" },
